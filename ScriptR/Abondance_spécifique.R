## Graphique montrant l'abondance spécifique 
...
sf_data$couleur <- as.factor(sf_data$couleur)
str(sf_data)

# Créer un quadrillage
grid <- st_make_grid(st_geometry(countries), cellsize = c(5, 5))

# Regarder le nombre de point entre chaque case de la grille et les données
indices <- st_intersects(grid, sf_data)
unique_species_counts <- vector("list", length = length(indices))

# Parcourir chaque cellule et compter le nombre d'espèces uniques
for (i in seq_along(indices)) {
  unique_species_counts[[i]] <- length(unique(sf_data$espece[indices[[i]]]))
}
b <- unlist(unique_species_counts)
print(b)

# Tracer la carte avec des points colorés en fonction de l'abondance spécifique
ggplot() +
  geom_sf(data = countries, fill = "808080") +
  geom_sf(data = grid, aes(fill=b),color = "#666666") +
  scale_fill_gradient(low = "yellow", high = "red", name = "Density",limits = c(1, max(b)),na.value = NA)+
  theme_void()
